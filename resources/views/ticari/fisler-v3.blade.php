                ...
                <tbody>
                    @forelse ($fisler as $fis)
                        <tr>
                            ...
                            <td><div class="d-flex gap-1 flex-wrap"><button type="button" class="btn btn-sm btn-outline-primary" onclick="document.getElementById('fis-duzenle-{{ $fis->id }}').showModal()">Düzenle</button><a class="btn btn-sm btn-outline-secondary" target="_blank" href="{{ route('ciktilar.yazdir', ['tur' => 'fis', 'id' => $fis->id]) }}">Yazdır</a><form method="POST" action="{{ route('ticari.fisler.sil',['fis'=>$fis->id,'firma_id'=>$firmaId]) }}" onsubmit="return confirm('Bu muhasebe fişi silinsin mi? Cari bakiyesi otomatik düzeltilecektir.')">@csrf @method('DELETE')<button class="btn btn-sm btn-outline-danger">Sil</button></form></div><dialog id="fis-duzenle-{{ $fis->id }}" class="fis-detail-dialog"><div class="fis-detail-content"><form method="POST" action="{{ route('ticari.fisler.guncelle',['fis'=>$fis->id,'firma_id'=>$firmaId]) }}">@csrf @method('PUT')<div class="fis-detail-heading"><h3>Fişi düzenle</h3><button type="button" class="btn-close" onclick="this.closest('dialog').close()"></button></div><div class="fis-detail-meta"><div><span class="fis-detail-label">Fiş no</span><input name="fis_no" value="{{ $fis->fis_no }}" required></div><div><span class="fis-detail-label">Fiş tarihi</span><input type="date" name="fis_tarihi" value="{{ $fis->fis_tarihi }}" required></div><div class="wide"><span class="fis-detail-label">Cari hesap</span><select name="cari_hesap_id"><option value="">Cari seçilmedi</option>@foreach($cariler as $cari)<option value="{{ $cari->id }}" @selected($fis->cari_hesap_id==$cari->id)>{{ $cari->unvan }}</option>@endforeach</select></div><div class="wide"><span class="fis-detail-label">Açıklama</span><input name="aciklama" value="{{ $fis->aciklama }}"></div><div><span class="fis-detail-label">Toplam tutar</span><input type="number" step=".01" min=".01" name="tutar" value="{{ $fis->tutar }}" required></div></div><button class="btn btn-primary w-100 mt-3">Değişiklikleri Kaydet</button></form></div></dialog></td>
                        </tr>
