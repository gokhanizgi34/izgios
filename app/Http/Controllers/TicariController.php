...
class TicariController extends Controller
{
    ...
    {
        $v=$request->validate(['firma_id'=>['required','exists:firmas,id'],'fis_no'=>['nullable','string','max:80','unique:muhasebe_fisleri,fis_no'],'cari_hesap_id'=>['nullable','exists:cari_hesaplar,id'],'fis_tarihi'=>['required','date'],'aciklama'=>['nullable','string','max:1000'],'satirlar'=>['required','array','min:1'],'satirlar.*.urun_adi'=>['required','string','max:255'],'satirlar.*.birim_fiyat'=>['required','numeric','gt:0'],'satirlar.*.kdv_orani'=>['required','numeric','min:0','max:100'],'satirlar.*.kdv_dahil_tutar'=>['required','numeric','gt:0']]);$v['yon']='gider';$v['tip']='Gider Fişi';$id=$this->firmaId($request);
    ...
    public function apiKaydet(Request $request){$v=$request->validate(['firma_id'=>['required','exists:firmas,id'],'saglayici'=>['required','in:logo,parasut,efatura,openai'],'api_anahtari'=>['nullable','string','max:1000']]);$this->firmaId($request);$mevcut=DB::table('muhasebe_entegrasyonlari')->where(['firma_id'=>$v['firma_id'],'saglayici'=>$v['saglayici']])->first();$anahtar=filled($v['api_anahtari'])?Crypt::encryptString($v['api_anahtari']):$mevcut?->api_anahtari_sifreli;DB::table('muhasebe_entegrasyonlari')->updateOrInsert(['firma_id'=>$v['firma_id'],'saglayici'=>$v['saglayici']],['api_anahtari_sifreli'=>$anahtar,'aktif'=>filled($anahtar),'durum'=>filled($anahtar)?'yapilandirildi':'yapilandirilmamis','updated_at'=>now(),'created_at'=>now()]);return back()->with('success','API ayarı şifreli olarak kaydedildi.');}
}
