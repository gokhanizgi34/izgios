...
class OperasyonController extends Controller {
    private function firma(Request $r, array $roller): int {
        abort_unless(auth()->check() && (auth()->user()->tamSistemYetkisiVarMi() || auth()->user()->isAdmin() || in_array(auth()->user()->role, $roller, true)), 403);
        $id=auth()->user()->tamSistemYetkisiVarMi() ? ($r->integer('firma_id') ?: Firma::where('aktif',true)->value('id')) : auth()->user()->firmaPersoneli?->firma_id;
        abort_unless($id && Firma::whereKey($id)->where('aktif',true)->exists(),403); return $id;
    }
    private function veri(int $firmaId): array {
    ...
    public function b2bKaydet(Request $r){$firmaId=$this->firma($r,['yedek_parca']);$v=$r->validate(['alici_unvan'=>'required|max:255','stok_parca_id'=>'nullable','urun_adi'=>'required|max:255','miktar'=>'required|numeric|gt:0','birim_fiyat'=>'required|numeric|min:0','durum'=>'required|in:taslak,onaylandi,hazirlaniyor,kargoda,tamamlandi,iptal']);DB::transaction(function()use($v,$firmaId){$tutar=$v['miktar']*$v['birim_fiyat'];$id=DB::table('b2b_siparisler')->insertGetId(['firma_id'=>$firmaId,'siparis_no'=>'B2B-'.now()->format('YmdHis'),'alici_unvan'=>$v['alici_unvan'],'durum'=>$v['durum'],'toplam_tutar'=>$tutar,'created_at'=>now(),'updated_at'=>now()]);DB::table('b2b_siparis_satirlari')->insert(['b2b_siparis_id'=>$id,'stok_parca_id'=>$v['stok_parca_id']??null,'urun_adi'=>$v['urun_adi'],'miktar'=>$v['miktar'],'birim_fiyat'=>$v['birim_fiyat'],'created_at'=>now(),'updated_at'=>now()]);});return back()->with('success','B2B sipariş kaydedildi.');}
}
